...
@section('title', 'Data Peserta Performance - KMDGI 16')
    ...
    <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-grow flex flex-col lg:flex-row gap-8 py-8 font-sans">
        ...
        <main class="flex-grow space-y-6 w-full">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Data Peserta Performance</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Daftar seluruh peserta/penonton tiket Penampil yang sudah berstatus Aktif.</p>
                </div>
                
                <!-- TOMBOL EXPORT -->
                <a href="{{ route('admin.peserta.performance.export', request()->query()) }}" class="inline-flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-5 rounded-xl text-sm transition-colors shadow-sm shadow-emerald-500/20 flex-shrink-0">
            ...
            <!-- Form Filter -->
            <form method="GET" action="{{ route('admin.peserta.performance.index') }}" class="bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Filter Penampil</label>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)] overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700 text-[14px]">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex flex-col">
                                        <span class="text-[11px] font-semibold text-[#FF6B9E] bg-[#FF6B9E]/10 px-2 py-0.5 rounded uppercase w-fit mb-1">{{ $peserta->jenis_tiket }}</span>
                                        <span class="text-sm font-bold text-slate-800 line-clamp-1" title="{{ $peserta->penampil->nama_penampil }}">{{ $peserta->penampil->nama_penampil }}</span>
                            ...
                            <tr>
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-sm text-slate-400 font-medium">Tidak ada data peserta performance.</span>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                ...
                @if($pesertaPerformances->hasPages())
                <div class="p-5 bg-slate-50/70 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 font-medium">
                    <span>Menampilkan {{ $pesertaPerformances->firstItem() ?? 0 }}-{{ $pesertaPerformances->lastItem() ?? 0 }} dari {{ $pesertaPerformances->total() }} Peserta</span>
                    <div class="flex gap-1.5">
                        <a href="{{ $pesertaPerformances->previousPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ $pesertaPerformances->onFirstPage() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Sebelumnya</a>
                        <a href="{{ $pesertaPerformances->nextPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ !$pesertaPerformances->hasMorePages() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Selanjutnya</a>
                    </div>
                </div>
                @endif
            </div>
