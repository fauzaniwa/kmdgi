        ...
        <main class="flex-grow space-y-6 w-full font-sans">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">Kelola informasi institusi, kontak perwakilan, dan status keanggotaan KMDGI.</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700 text-[14px]">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200/60 p-1.5 flex-shrink-0 flex items-center justify-center relative overflow-hidden" 
                                             style="background-image: radial-gradient(#e2e8f0 1px, transparent 1px); background-size: 8px 8px;">
                                ...
                                <td class="py-4 px-4 space-y-1.5">
                                    ...
                                    <div class="flex items-center gap-1.5 text-xs">
                                        ...
                                        <a href="https://instagram.com/{{ ltrim($kampus->medsos_kampus, '@') }}" target="_blank" class="text-kmdgi-primary hover:underline font-medium truncate max-w-[120px]">
                                            {{ '@' . ltrim($kampus->medsos_kampus, '@') }}
                                        </a>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex items-center gap-1.5 text-xs">
                                        ...
                                        <a href="https://instagram.com/{{ ltrim($kampus->ig_prodi, '@') }}" target="_blank" class="text-slate-500 hover:text-kmdgi-primary hover:underline font-medium truncate max-w-[120px]">
                                            {{ '@' . ltrim($kampus->ig_prodi, '@') }}
                                        </a>
                                    </div>
                                    @endif
...
<!-- ========================================== -->
<div id="modal-form-kampus" class="fixed inset-0 z-[80] hidden opacity-0 transition-opacity duration-300 ease-in-out flex items-center justify-center">
    <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onclick="closeKampusModal()"></div>
    
    <div class="relative bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-[32rem] mx-4 shadow-2xl transform scale-95 transition-transform duration-300 ease-in-out box-form max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <form id="kampus-form" action="{{ route('admin.kampus.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div id="method-container"></div>

            <!-- Interaktif Live Preview Logo Upload -->
            <div>
                ...
                <div class="relative w-full h-32 border-2 border-dashed border-slate-300 hover:border-kmdgi-primary rounded-2xl bg-slate-50 flex items-center justify-center overflow-hidden group transition-colors cursor-pointer" onclick="document.getElementById('input-logo').click()">
                    ...
                    <div id="upload-placeholder" class="flex flex-col items-center pointer-events-none relative z-10 transition-opacity">
                        <svg class="w-8 h-8 text-slate-400 mb-2 group-hover:text-kmdgi-primary transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div class="flex justify-between items-center mt-1.5">
                    <p class="text-[11px] text-slate-400">Gunakan file bereksistensi .PNG latar transparan. Max 2MB.</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">User IG Kampus <span class="text-slate-300 lowercase font-normal">(Opsional)</span></label>
                    <input type="text" name="medsos_kampus" id="input-medsos" placeholder="Misal: itb1920" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-kmdgi-primary focus:bg-white transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">User IG Prodi <span class="text-slate-300 lowercase font-normal">(Opsional)</span></label>
                    <input type="text" name="ig_prodi" id="input-prodi" placeholder="Misal: fsrditb" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-kmdgi-primary focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Status Keanggotaan</label>
                    <select name="status_keanggotaan" id="input-status" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-kmdgi-primary focus:bg-white transition-all cursor-pointer">
...
<script>
    ...
    function openKampusModal(action, data = null) {
        ...
        if (action === 'create') {
            ...
            // Clean logic to ensure `@` does not appear in the input field
            ...
            document.getElementById('input-status').value = data.status_keanggotaan;
