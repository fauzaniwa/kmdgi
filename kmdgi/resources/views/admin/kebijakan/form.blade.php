...
@section('title', (isset($kebijakan) ? 'Edit' : 'Tambah') . ' Kebijakan Privasi - KMDGI 16')
...
<div class="bg-slate-50 min-h-screen flex flex-col">
    ...
    <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-grow flex flex-col lg:flex-row gap-8 py-8">
        ...
        <main class="flex-grow space-y-6 w-full font-sans">
            ...
            <a href="{{ route('admin.kebijakan.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-kmdgi-primary transition-colors">
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div class="mb-8">
                    <h2 class="text-2xl font-bold text-slate-900 tracking-tight">{{ isset($kebijakan) ? 'Edit Kebijakan Privasi' : 'Buat Kebijakan Privasi' }}</h2>
                    <p class="text-sm text-slate-500 mt-1">Gunakan editor teks di bawah ini untuk merumuskan kebijakan privasi platform KMDGI.</p>
                </div>

                <form id="kebijakan-form" action="{{ isset($kebijakan) ? route('admin.kebijakan.update', $kebijakan->id) : route('admin.kebijakan.store') }}" method="POST" class="space-y-6">
                    @csrf
                    @if(isset($kebijakan))
                        @method('PUT')
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Judul Kebijakan <span class="text-red-500">*</span></label>
                            <input type="text" name="judul" value="{{ old('judul', $kebijakan->judul ?? '') }}" required class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-kmdgi-primary focus:bg-white transition-all" placeholder="Cth: Kebijakan Pengumpulan Data Pengguna">
                        </div>
                        <div>
                            ...
                            <select name="is_active" required class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-kmdgi-primary focus:bg-white transition-all cursor-pointer">
                                <option value="1" {{ old('is_active', $kebijakan->is_active ?? 1) == 1 ? 'selected' : '' }}>Aktif (Tampilkan)</option>
                                <option value="0" {{ old('is_active', $kebijakan->is_active ?? 1) == 0 ? 'selected' : '' }}>Draft (Sembunyikan)</option>
                            </select>
                        </div>
                    </div>

                    <div class="flex flex-col flex-grow">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Isi Kebijakan <span class="text-red-500">*</span></label>
                        ...
                        <div class="flex-grow flex flex-col border border-slate-200 rounded-xl overflow-hidden focus-within:border-kmdgi-primary focus-within:ring-1 focus-within:ring-kmdgi-primary/30 transition-all bg-white min-h-[400px]">
                            ...
                            <div id="quill-editor" class="flex-grow h-full text-[15px] text-slate-700 p-2">{!! old('konten', $kebijakan->konten ?? '') !!}</div>
                        </div>
                    </div>

                    <div class="pt-6 border-t border-slate-100 flex flex-col-reverse sm:flex-row justify-end gap-3">
                        <a href="{{ route('admin.kebijakan.index') }}" class="inline-flex justify-center bg-white border border-slate-200 text-slate-600 font-bold py-3.5 px-8 rounded-xl hover:bg-slate-50 transition-colors text-sm shadow-sm">Batalkan</a>
                        <button type="submit" class="inline-flex justify-center bg-kmdgi-primary hover:bg-kmdgi-hover text-white font-bold py-3.5 px-8 rounded-xl transition-colors text-sm shadow-md shadow-kmdgi-primary/20">
                            {{ isset($kebijakan) ? 'Simpan Perubahan' : 'Buat Dokumen Sekarang' }}
                        </button>
                    </div>
                </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const quill = new Quill('#quill-editor', {
            ...
            placeholder: 'Ketikkan rincian kebijakan privasi di sini...',
            theme: 'snow'
        });

        document.getElementById('kebijakan-form').addEventListener('submit', function(e) {
