...
@section('title', 'Verifikasi Tiket Performance - KMDGI 16')
    ...
    <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-grow flex flex-col lg:flex-row gap-8 py-8 font-sans">
        ...
        <main class="flex-grow space-y-6 w-full">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Verifikasi Tiket Performance</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Daftar pembayaran tiket Guest Star yang <span class="font-bold text-amber-500">Menunggu Konfirmasi</span>.</p>
                </div>
            </div>

            <form method="GET" action="{{ route('admin.verifikasi.performance.index') }}" class="bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Pilih Penampil</label>
                        <select name="penampil_id" onchange="this.form.submit()" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none cursor-pointer">
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)] overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-100 bg-slate-50/70 text-slate-400 font-bold text-xs uppercase tracking-wider">
                                <th class="py-4 px-6 min-w-[200px]">Peserta & Penampil</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-slate-700 text-[14px]">
                            @forelse($tiketPerformances as $tiket)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-4 px-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-[11px] text-slate-500 block truncate max-w-[200px] mt-0.5" title="{{ $tiket->penampil->nama_penampil }}">{{ $tiket->penampil->nama_penampil }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <form action="{{ route('admin.verifikasi.performance.approve', $tiket->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            <button type="button" 
                                                onclick="openModal('kmdgi-global-modal', this)"
                                                data-title="Setujui Pembayaran?"
                                                data-message="Pastikan dana Rp {{ number_format($tiket->penampil->harga_tiket, 0, ',', '.') }} dari {{ $tiket->user->name }} sudah masuk ke rekening panitia."
                                                data-type="success"
                                        ...
                                        <form id="approve-form-{{ $tiket->id }}" action="{{ route('admin.verifikasi.performance.approve', $tiket->id) }}" method="POST" class="hidden">
                                            @csrf
                                        </form>

                                        <form action="{{ route('admin.verifikasi.performance.destroy', $tiket->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
                                                onclick="openModal('kmdgi-global-modal', this)"
                                        ...
                                        <form id="delete-form-{{ $tiket->id }}" action="{{ route('admin.verifikasi.performance.destroy', $tiket->id) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
                            ...
                            <tr>
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm text-slate-500">Tidak ada tiket Performance yang menunggu verifikasi saat ini.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                ...
                <div class="p-5 bg-slate-50/70 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 font-medium">
                    <span>Menampilkan {{ $tiketPerformances->firstItem() ?? 0 }}-{{ $tiketPerformances->lastItem() ?? 0 }} dari {{ $tiketPerformances->total() }} Menunggu</span>
                    <div class="flex gap-1.5">
                        <a href="{{ $tiketPerformances->previousPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ $tiketPerformances->onFirstPage() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Sebelumnya</a>
                        <a href="{{ $tiketPerformances->nextPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ !$tiketPerformances->hasMorePages() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Selanjutnya</a>
                    </div>
                </div>
                @endif
            </div>
