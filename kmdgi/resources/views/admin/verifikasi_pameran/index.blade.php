...
@section('title', 'Verifikasi Tiket Pameran - KMDGI 16')
    ...
    <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-grow flex flex-col lg:flex-row gap-8 py-8 font-sans">
        ...
        <main class="flex-grow space-y-6 w-full">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Verifikasi Tiket Pameran</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Daftar pembayaran tiket Pameran yang <span class="font-bold text-amber-500">Menunggu Konfirmasi</span>.</p>
                </div>
            </div>

            <form method="GET" action="{{ route('admin.verifikasi.pameran.index') }}" class="bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm space-y-4">
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)] overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-100 bg-slate-50/70 text-slate-400 font-bold text-xs uppercase tracking-wider">
                                <th class="py-4 px-6 min-w-[200px]">Data Peserta</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-4 px-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-[11px] font-semibold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded uppercase mt-1 inline-block">Tiket Pameran</span>
                                            <span class="text-[11px] text-slate-500 block mt-0.5">Kode: <span class="font-mono text-slate-700">{{ $tiket->kode_tiket }}</span></span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <form action="{{ route('admin.verifikasi.pameran.approve', $tiket->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            <button type="button" 
                                                onclick="openModal('kmdgi-global-modal', this)"
                                                data-title="Setujui Pembayaran?"
                                                data-message="Pastikan dana dari {{ $tiket->user->name }} sudah masuk ke rekening panitia sebelum mengaktifkan tiket ini."
                                                data-type="success"
                                        ...
                                        <form id="approve-form-{{ $tiket->id }}" action="{{ route('admin.verifikasi.pameran.approve', $tiket->id) }}" method="POST" class="hidden">
                                            @csrf
                                        </form>

                                        <form action="{{ route('admin.verifikasi.pameran.destroy', $tiket->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" 
                                                onclick="openModal('kmdgi-global-modal', this)"
                                        ...
                                        <form id="delete-form-{{ $tiket->id }}" action="{{ route('admin.verifikasi.pameran.destroy', $tiket->id) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
                            ...
                            <tr>
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm text-slate-500">Tidak ada tiket Pameran yang menunggu verifikasi saat ini.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                ...
                <div class="p-5 bg-slate-50/70 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 font-medium">
                    <span>Menampilkan {{ $tiketPameran->firstItem() ?? 0 }}-{{ $tiketPameran->lastItem() ?? 0 }} dari {{ $tiketPameran->total() }} Menunggu</span>
                    <div class="flex gap-1.5">
                        <a href="{{ $tiketPameran->previousPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ $tiketPameran->onFirstPage() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Sebelumnya</a>
                        <a href="{{ $tiketPameran->nextPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ !$tiketPameran->hasMorePages() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl">Selanjutnya</a>
                    </div>
                </div>
                @endif
            </div>
