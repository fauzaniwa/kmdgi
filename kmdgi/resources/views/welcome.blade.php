...
<style>
    ...
    .marquee-track { display: flex; gap: 2rem; animation: scroll-marquee 30s linear infinite; }
...
<div class="relative w-full min-h-screen bg-white font-sans flex flex-col overflow-x-hidden">
    ...
    <section class="py-20 md:py-28 bg-white relative z-20" id="acara">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-end mb-10 md:mb-14">
                ...
                <a href="#" class="text-sm font-semibold text-[#1A68FF] hover:text-blue-800 flex items-center gap-1.5 transition-colors">
                    Lihat Selengkapnya 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12">
                ...
                <div class="{{ $index >= 2 ? 'hidden md:flex' : 'flex' }} flex-col md:flex-row gap-6 w-full">
                    
                    <div class="w-full md:w-[220px] aspect-[4/5] md:aspect-auto rounded-[1.25rem] overflow-hidden bg-slate-100 flex-shrink-0 relative group">
                        @if($event->poster)
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center text-slate-400 text-xs text-center p-4">Gambar<br>Belum Tersedia</div>
                        @endif
                    </div>
                    ...
                    <div class="flex-1 flex flex-col justify-between py-1">
                        <div>
                            <div class="flex flex-wrap items-center gap-2 mb-3">
                                @if(is_array($event->kategori_peserta) && count($event->kategori_peserta) > 0)
                                    @foreach($event->kategori_peserta as $kat)
                                        <span class="px-3 py-1 rounded-full border border-emerald-200 text-emerald-700 bg-emerald-50 text-[11px] font-semibold tracking-wide">
                                            {{ $kat }}
                                        </span>
                                    @endforeach
                                @else
                                    <span class="px-3 py-1 rounded-full border border-emerald-200 text-emerald-700 bg-emerald-50 text-[11px] font-semibold tracking-wide">Umum</span>
                                @endif
                                
                                <span class="px-3 py-1 rounded-full border border-blue-200 text-[#1A68FF] bg-blue-50 text-[11px] font-semibold tracking-wide whitespace-nowrap">
                                    {{ $event->kuota == 0 ? 'Tak Terbatas' : $event->kuota . ' Orang' }}
                                </span>
                            </div>
                            
                            <h3 class="text-xl md:text-2xl font-bold text-slate-900 leading-snug mb-1.5 line-clamp-2" title="{{ $event->judul }}">
                                {{ $event->judul }}
                            </h3>
                            ...
                            <p class="text-sm text-slate-600 line-clamp-2 mb-4">
                                ...
                                @endphp
                                {{ $kolabNames ?: 'Narasumber Umum' }} <span class="text-slate-500">- {{ $eventKolaborators->first()->profesi ?? 'Profesional' }}</span>
                            </p>

                            <div class="space-y-2 mb-6">
                                <div class="flex items-center gap-2.5 text-sm text-slate-700">
                                    <svg class="w-4 h-4 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                                    {{ $event->jam_pelaksanaan ? \Carbon\Carbon::parse($event->jam_pelaksanaan)->format('H.i') : '-' }} WIB
                                </div>
                                <div class="flex items-center gap-2.5 text-sm text-slate-700">
                                    <svg class="w-4 h-4 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" /></svg>
                                    {{ $event->tanggal_pelaksanaan ? \Carbon\Carbon::parse($event->tanggal_pelaksanaan)->locale('id')->translatedFormat('d F Y') : '-' }}
                                </div>
                                <div class="flex items-center gap-2.5 text-sm text-slate-700">
                        ...
                        @if($isRegistered)
                            <button disabled class="mt-auto block w-full bg-slate-100 text-slate-500 font-semibold py-2.5 rounded-[0.75rem] text-sm md:text-base cursor-not-allowed">
                                Terdaftar
                            </button>
                        @elseif($isFull)
                            <button disabled class="mt-auto block w-full bg-red-50 text-red-500 font-semibold py-2.5 rounded-[0.75rem] text-sm md:text-base cursor-not-allowed border border-red-100">
                                Kuota Penuh
                            </button>
                        @else
                            <a href="{{ url('/event/' . $event->slug) }}" class="mt-auto block w-full bg-[#1A68FF] hover:bg-blue-700 text-white text-center font-semibold py-2.5 rounded-[0.75rem] text-sm md:text-base transition-colors shadow-sm">
                                Daftar
                            </a>
                        @endif
                    </div>
    ...
    <section class="py-20 md:py-28 bg-slate-50 relative z-20 border-t border-slate-100" id="performance">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @foreach($penampils as $tanggal => $group)
                <div class="mb-14">
                    <div class="flex items-center gap-4 mb-8">
                        <h3 class="text-xl md:text-2xl font-bold text-slate-900 whitespace-nowrap">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                        ...
                        <div class="flex flex-row gap-4 md:gap-6 w-full">
                            <div class="w-1/3 md:w-[200px] aspect-square flex-shrink-0">
                                @if($penampil->cover_penampil)
                                    <img src="{{ asset('storage/' . $penampil->cover_penampil) }}" alt="{{ $penampil->nama_penampil }}" class="w-full h-full object-cover rounded-[1.25rem] shadow-sm">
                                @elseif($penampil->logo_penampil)
                                    <img src="{{ asset('storage/' . $penampil->logo_penampil) }}" alt="{{ $penampil->nama_penampil }}" class="w-full h-full object-contain p-2 bg-white rounded-[1.25rem] shadow-sm">
                                @else
                                    <div class="w-full h-full rounded-[1.25rem] bg-slate-200 border border-slate-300 flex items-center justify-center text-slate-400 text-[10px] md:text-xs">No Image</div>
                                @endif
                            </div>
                            ...
                            <div class="w-2/3 flex flex-col justify-between py-1">
                                <div>
                                    <div class="flex items-center gap-2 mb-2.5">
                                        <span class="px-3 py-1 rounded-full border border-emerald-200 text-emerald-700 bg-emerald-50 text-[10px] md:text-[11px] font-semibold tracking-wide">
                                            {{ $penampil->kategori_penonton ?? 'Umum' }}
                                        </span>
                                        <span class="px-3 py-1 rounded-full border border-blue-200 text-[#1A68FF] bg-blue-50 text-[10px] md:text-[11px] font-semibold tracking-wide">
                                            {{ (strtolower($penampil->tipe_pendaftaran) == 'gratis' || $penampil->harga_tiket == 0) ? 'Free' : 'Rp ' . number_format($penampil->harga_tiket, 0, ',', '.') }}
                                        </span>
                                    </div>
                                    
                                    <h4 class="text-lg md:text-xl font-bold text-slate-900 leading-snug mb-3 md:mb-4 line-clamp-2">
                                        {{ $penampil->nama_penampil }}
                                    </h4>

                                    <div class="space-y-1.5 md:space-y-2 mb-4">
                                        <div class="flex items-center gap-2 text-xs md:text-sm text-slate-700">
                                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                                        ...
                                        <div class="flex items-center gap-2 text-xs md:text-sm text-slate-700">
                                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" /></svg>
                                            {{ $penampil->tanggal_tampil ? \Carbon\Carbon::parse($penampil->tanggal_tampil)->locale('id')->translatedFormat('d F Y') : '-' }}
                                        </div>
                                        <div class="flex items-center gap-2 text-xs md:text-sm text-slate-700">
                                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>
                                            <span class="truncate">{{ $penampil->lokasi_tampil ?? '-' }}</span>
                                        </div>
                                    </div>
                                </div>
                                
                                @if(strtolower($penampil->tipe_pendaftaran) !== 'gratis' && $penampil->harga_tiket > 0)
                                    @if($penampil->link_pendaftaran)
                                        <a href="{{ $penampil->link_pendaftaran }}" target="_blank" class="w-full md:w-[75%] bg-[#1A68FF] hover:bg-blue-700 text-white text-center font-semibold py-2 md:py-2.5 rounded-[0.75rem] text-xs md:text-sm transition-colors shadow-sm">
                                            Daftar
                                        </a>
                                    @else
                                        <button disabled class="w-full md:w-[75%] bg-slate-200/60 text-slate-400 font-semibold py-2 md:py-2.5 rounded-[0.75rem] text-xs md:text-sm cursor-not-allowed">
                                            Daftar
                                        </button>
                                    @endif
                                @endif
            ...
            <div class="mt-10 flex justify-center">
                <a href="#" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-[#1A68FF] text-[#1A68FF] hover:bg-blue-50 font-semibold text-sm transition-colors">
                    Lihat Selengkapnya 
    ...
    <section class="py-20 md:py-28 bg-white relative z-20 border-t border-slate-100" id="faq">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Card Latar Belakang Cerah (Rose/Slate Tint) -->
