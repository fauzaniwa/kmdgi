            ...
            <form id="form-submisi" action="{{ route('delegasi.submisi.store', $kategori) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-white rounded-[2rem] p-8 md:p-10 border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-slate-900 tracking-tight">Thumbnail (Preview Karya)</h3>
                            </div>
                        </div>

                        @if(!$isFinal)
                        <p class="text-sm text-slate-500 mb-6">Unggah gambar yang merepresentasikan karya Anda. Gambar ini akan digunakan untuk kebutuhan katalog dan galeri situs.</p>
                        @endif
                        ...
                        <div id="dropzone-thumbnail" onclick="document.getElementById('thumbnail_karya').click()" class="{{ $hasDraftThumb ? 'hidden' : 'flex' }} border-2 border-dashed border-slate-300 rounded-[1.5rem] p-10 flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all group">
                            ...
                            <p class="text-sm text-slate-700 mb-1"><span class="font-bold text-[#1A68FF]">Klik untuk unggah</span> atau seret dan lepas gambar</p>
                            <p class="text-xs text-slate-500">Mendukung format SVG, PNG, JPG, GIF (Maks. 5MB)</p>
                        </div>
                        @endif
                            ...
                            <div class="flex items-center gap-5">
                                ...
                                <div class="flex-grow">
                                    <div class="flex justify-between items-start mb-2.5">
                                        <div>
                                            <p id="filename-thumbnail" class="text-sm font-bold text-slate-900 truncate max-wxs md:max-w-md">{{ $hasDraftThumb ? basename($draft->thumbnail_karya) : 'Tidak ada gambar' }}</p>
                                            <p class="text-[11px] font-medium text-slate-500 flex items-center gap-2 mt-0.5">
                                                <span id="filesize-thumbnail">{{ $hasDraftThumb ? ($isFinal ? 'Berkas Final' : 'Draft Tersimpan') : '' }}</span>
                                            </p>
                                        </div>
                    ...
                    <div class="bg-blue-50/50 rounded-[2rem] p-8 md:p-10 border border-blue-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                        ...
                        <div class="bg-white rounded-[1.5rem] border border-slate-200 p-6 md:p-8 shadow-sm">
                            ...
                            <div>
                                ...
                                @if(!$isFinal)
                                <p class="text-[11px] text-amber-600 font-medium mt-2">Pastikan akses link G-Drive/Figma/dsb telah di-set "Anyone with the link can view".</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <div id="dropzone-file" onclick="document.getElementById('file_karya').click()" class="{{ $hasDraftFile ? 'hidden' : 'flex' }} border-2 border-dashed border-slate-300 rounded-[1.5rem] p-8 flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all group">
                                    ...
                                    <p class="text-[11px] text-slate-500">Mendukung format: ZIP, PDF, JPG, PNG.</p>
                                </div>
                                @endif
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-10">
                    <button type="button"
                        ...
                        data-message="Data Anda akan disimpan sementara. Anda bisa kembali untuk melengkapinya nanti sebelum mengirim secara final."
                        data-type="info"
                        data-primary-text="Ya, Simpan Draft"
                        data-secondary-text="Batal"
                        data-form-id="form-submisi"
                        class="flex items-center justify-center gap-2 bg-white text-slate-600 border border-slate-200 font-bold text-[13px] px-8 py-4 rounded-xl hover:bg-slate-50 hover:text-slate-800 transition-colors focus:outline-none">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
                        </svg>
                        Simpan Draft
                    ...
                    <button type="button"
                        onclick="triggerSubmitModal(false, this)"
                        data-title="Kirim Submisi Final?"
                        data-message="Pastikan Anda telah mengisi Tautan URL ATAU mengunggah File Karya. Data yang sudah dikirim secara final tidak dapat diubah kembali."
                        data-type="success"
                        data-primary-text="Kirim Final"
                        data-secondary-text="Batal"
                        data-form-id="form-submisi"
                        class="bg-[#1A1A1A] text-white font-bold text-[13px] px-10 py-4 rounded-xl hover:bg-black transition-all shadow-md focus:outline-none">
                        Kirim Submisi Final
                    </button>
                </div>
                @endif
...
<script>
    ...
    function closeFilePreview() {
        ...
        setTimeout(() => {
            ...
            document.getElementById('previewContainer').innerHTML = ''; // Hapus dari memori DOM
        }, 300);
    }

    // Hanya inisiasi script upload input jika form belum di-disable (belum final)
    @if(!$isFinal)
    ...
    function handleFileSelect(input, type) {
        ...
        if (input.files && input.files[0]) {
            ...
            filesizeLabel.innerText = formatBytes(file.size);
