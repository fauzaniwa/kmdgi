...
@section('title', 'Rekap Kehadiran Performance - KMDGI 16')
    ...
    <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex-grow flex flex-col lg:flex-row gap-8 py-8">
        ...
        <main class="flex-grow space-y-6 w-full min-w-0 font-sans">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)]">
                <div>
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Rekap Kehadiran Performance</h2>
                    <p class="text-xs text-slate-400 mt-0.5">Daftar peserta yang telah divalidasi masuk (Hadir) untuk tiket Performance konser/penampil.</p>
                </div>
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-600 border border-emerald-100">
                        ...
                        Total Hadir: {{ $kehadiranPerformance->total() }}
                    </span>
                </div>
            </div>

            <!-- Bar Pencarian & Filter -->
            <form method="GET" action="{{ route('admin.kehadiran.performance.index') }}" class="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)] flex flex-col md:flex-row gap-3 items-center justify-between">
                ...
                <div class="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full md:w-auto">
                    <select name="penampil_id" class="w-full sm:w-auto px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:border-kmdgi-primary transition-colors">
                        <option value="all">Semua Penampil</option>
                        @foreach($listPenampil as $penampil)
                        <option value="{{ $penampil->id }}" {{ request('penampil_id') == $penampil->id ? 'selected' : '' }}>{{ $penampil->nama_penampil }}</option>
                        @endforeach
                    </select>

                    <button type="submit" class="w-full sm:w-auto px-5 py-2.5 bg-kmdgi-primary hover:bg-kmdgi-hover text-white text-sm font-semibold rounded-xl transition-all shadow-sm">Filter</button>
                    
                    <!-- TOMBOL EXPORT EXCEL -->
                    <a href="{{ route('admin.kehadiran.performance.export', request()->query()) }}" class="w-full sm:w-auto px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-xl transition-all shadow-sm text-center flex items-center justify-center gap-2">
                    ...
                    @if(request()->filled('search') || request()->filled('penampil_id'))
                    <a href="{{ route('admin.kehadiran.performance.index') }}" class="text-xs text-slate-400 hover:text-red-500 font-semibold px-2 py-1">Reset</a>
                    @endif
                </div>
            </form>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.005)] w-full max-w-full overflow-hidden">
                    ...
                    <table class="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                            <tr class="border-b border-slate-100 bg-slate-50/70 text-slate-400 font-bold text-xs uppercase tracking-wider">
                                ...
                                <th class="py-4 px-4">Nama Penampil (Konser)</th>
                                <th class="py-4 px-6 text-center">Waktu Scan Masuk</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-slate-700 text-[14px]">
                            @forelse($kehadiranPerformance as $index => $tiket)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="py-4 px-6 text-center font-medium text-slate-400">{{ $kehadiranPerformance->firstItem() + $index }}</td>
                ...
                <div class="p-5 bg-slate-50/70 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 font-medium">
                    <span>Menampilkan {{ $kehadiranPerformance->firstItem() ?? 0 }}-{{ $kehadiranPerformance->lastItem() ?? 0 }} dari {{ $kehadiranPerformance->total() }} Hadir</span>
                    <div class="flex gap-1.5">
                        <a href="{{ $kehadiranPerformance->previousPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ $kehadiranPerformance->onFirstPage() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl transition-colors shadow-sm">Sebelumnya</a>
                        <a href="{{ $kehadiranPerformance->nextPageUrl() }}" class="px-3 py-2 bg-white border border-slate-200 {{ !$kehadiranPerformance->hasMorePages() ? 'text-slate-400 pointer-events-none' : 'text-slate-700 hover:text-kmdgi-primary' }} rounded-xl transition-colors shadow-sm">Selanjutnya</a>
                    </div>
                </div>
            </div>
