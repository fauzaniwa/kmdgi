...
<!-- ========================================== -->
@if($event->harga_tiket > 0 || (isset($event->harga_tiket) && $event->harga_tiket == 0))
...
<div id="daftarModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-slate-900/80 backdrop-blur-sm transition-all duration-300 opacity-0 pointer-events-none overflow-y-auto">
    <div class="bg-white rounded-[2rem] overflow-hidden w-full max-w-xl shadow-2xl transform transition-all scale-95 my-auto" id="daftarModalContent">
        ...
        <div class="bg-slate-50 border-b border-slate-100 px-8 py-5 flex justify-between items-center sticky top-0 z-20">
        ...
        <form action="{{ route('katalog.event.daftar', $event->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 mb-8 text-center shadow-inner">
                ...
                <span class="text-3xl font-black text-indigo-700 block drop-shadow-sm">Rp {{ number_format($event->harga_tiket, 0, ',', '.') }}</span>
                <p class="text-xs text-indigo-600 mt-2 font-medium">Tiket Acara: <strong>{{ $event->judul }}</strong></p>
            </div>
