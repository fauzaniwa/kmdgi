            ...
            <div class="w-full lg:w-[55%] flex flex-col">
                ...
                <div class="bg-[#FFFDFD] rounded-[1.5rem] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 mb-10 w-full md:max-w-md">
                    ...
                    <div class="border-t border-slate-200 mt-6 pt-6">
                        ...
                        @if($isFree)
                            <!-- Jika Gratis, TIDAK ADA TOMBOL MENDAFTAR -->
                        ...
                        @else
                            <!-- Jika Berbayar, Tampilkan Logika Tombol Klaim/Beli Tiket -->
                            @if(!Auth::check())
...
<div id="daftarModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-slate-900/80 backdrop-blur-sm transition-all duration-300 opacity-0 pointer-events-none overflow-y-auto">
    <div class="bg-white rounded-[2rem] overflow-hidden w-full max-w-xl shadow-2xl transform transition-all scale-95 my-auto" id="daftarModalContent">
        ...
        <div class="bg-slate-50 border-b border-slate-100 px-8 py-5 flex justify-between items-center">
            <h3 class="text-lg font-black text-slate-900">Konfirmasi Tiket Performance</h3>
        ...
        <form action="{{ route('performance.daftar', \Illuminate\Support\Str::slug($penampil->nama_penampil)) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf

            <p class="text-sm text-slate-600 mb-6">Anda akan mengklaim tiket untuk penampilan <strong>{{ $penampil->nama_penampil }}</strong>.</p>
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 mb-6 text-center">
                ...
                <span class="text-3xl font-black text-indigo-700 block mb-4">Rp {{ number_format($penampil->harga_tiket, 0, ',', '.') }}</span>
